...
| - | - |
| ![Application homepage view in desktop layout.](public/screens/desktop-homepage-view.png) | ![Application homepage view in mobile layout.](public/screens/mobile-homepage-view.png) |
...
### Graphic elements
The Art Collecor logo and 404 error handling page illustration are self-prepared. Project includes icons from [Remix Icon](https://remixicon.com/) library.
...
The application uses custom infinie scroll solution for dynamic load of additional artworks in the ```<ArtworkList>``` component. The infinite scroll effect is triggered when the user reaches the bottom of the ```<RootLayout>``` component. Additional dependencies check weather search term contains more pages, weather all works have been loaded, and weather loading is currently in progress.
...
After dispatching actions like adding artwork to favourites list or creating new collection, user receives confirmation in form of a notification prompt. Notification state is managed by the custom hook, which returns ```showNotification()``` function.  Hook controlls notification timeout and prevents notification stacking.
...
Artworks data used in application is provided by [Artic API](http://api.artic.edu/docs/). API passes data in ```.JSON``` format. Application utilizes Axios library to simplify the fetching data process. Data is accesed via API endpoits. 
...
| - | - |
| ![Step 1 before optimization.](public/screens/before-optimization-step-1.png) | ![Step 1 after the optimization.](public/screens/after-optimization-step-3.png) |
...
Other optimization methods were not found to be significaly beneficial. Since the application does not repeatedly perform complex calculations, there would be no benefit from using the ```useMemo()```  hook.

Unoptimized code repository is avaliable in _"Start optimizing"_ commit. Optimized code repozitory is avaliable in _"Optimized RootLayout and ArtworkList (prevent unnecesary rerenders)"_.
...
- Code
    ...
    - Restructuring notification custom hook for keeping state in one place and better reusability. Create new hooks to abstract imlementation details.
- Features
    - more detailed search (specific author, technique, etc.)
    - fitering artworks list
...
- API
    - [Art Insitute of Chicago API](http://api.artic.edu)
- Licence 
